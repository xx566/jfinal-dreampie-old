...
public class UserValidator {
  ...
  public static class addFollowingValidator extends Validator {
    ...
    protected void validate(Controller c) {
      ...
      boolean idEmpty = cn.dreampie.common.util.ValidateUtils.me().isNullOrEmpty(c.getPara("follower.id"));
      if (idEmpty) addError("idMsg", "联系人参数异常");
      if (!idEmpty && !cn.dreampie.common.util.ValidateUtils.me().isPositiveNumber(c.getPara("follower.id")))
        addError("idMsg", "联系人参数异常");
      if (!idEmpty) {
        Follower con = Follower.dao.findById(c.getPara("follower.id"));
        if (cn.dreampie.common.util.ValidateUtils.me().isNullOrEmpty(con))
          addError("idMsg", "联系人不存在");
      }
      boolean introEmpty = cn.dreampie.common.util.ValidateUtils.me().isNullOrEmpty(c.getPara("follower.intro"));
      if (introEmpty) addError("introMsg", "备注不能为空");
      if (!introEmpty && !cn.dreampie.common.util.ValidateUtils.me().isLength(c.getPara("follower.intro"), 3, 240))
  ...
  public static class UpdatePwdValidator extends Validator {
    ...
    protected void validate(Controller c) {
      ...
      if (cn.dreampie.common.util.ValidateUtils.me().isNullOrEmpty(User.dao.findBy("`user`.id=" + c.getPara("user.id"))))
        addError("user_idMsg", "账户不存在");

      boolean userEmpty = cn.dreampie.common.util.ValidateUtils.me().isNullOrEmpty(c.getPara("user.username"));
      if (userEmpty) addError("user_usernameMsg", "账户丢失");
      if (!userEmpty && !cn.dreampie.common.util.ValidateUtils.me().isUsername(c.getPara("user.username")))
      ...
      boolean oldpasswordEmpty = cn.dreampie.common.util.ValidateUtils.me().isNullOrEmpty(c.getPara("oldpassword"));
      if (oldpasswordEmpty) addError("user_oldpasswordMsg", "原始密码不能为空");

      if (!oldpasswordEmpty && !cn.dreampie.common.util.ValidateUtils.me().isPassword(c.getPara("oldpassword")))
